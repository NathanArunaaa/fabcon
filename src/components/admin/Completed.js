    ...
    const fetchRequestsData = async () => {
      try {
        ...
        for (const orgId of orgIds) {
          ...
          const completedIds = orgData.completedIds;

          console.log("Request IDs for organization", orgId, ":", completedIds); // Log request IDs

          for (const completedId of completedIds) {
            const requestDoc = await getDoc(doc(db, "requests", completedId));
            if (!requestDoc.exists()) {
              console.error(`Request with ID ${completedId} does not exist.`);
              continue;
            }
